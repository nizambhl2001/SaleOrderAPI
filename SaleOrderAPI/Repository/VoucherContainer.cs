...
namespace SaleOrderAPI.Repository
{
    ...
    public class VoucherContainer : IVoucherContainer
    {

        private readonly Sales_DBContext _DBContext;
        private readonly IMapper mapper;
        public VoucherContainer(Sales_DBContext dBContext, IMapper mapper)
        {
        ...
        public async Task<List<InvoiceDetails>> GetAllInvoiceDetailbyCode(string invoiceno)
        {
            ...
            if (_data != null && _data.Count > 0)
            {
                return this.mapper.Map<List<TblSalesProductInfo>, List<InvoiceDetails>>(_data);
            }
            return new List<InvoiceDetails>();
        }

        public async Task<ResponsType> Save(InvoiceInput invoiceEntity)
        {
            string Result = string.Empty;
            int processcount = 0;
            var response = new ResponsType();
        ...
        private async Task<bool> SaveDetail(InvoiceDetails invoiceDetail, string User, string InvoiceNo)
        {
            try
            {
                TblSalesProductInfo _detail = this.mapper.Map<InvoiceDetails, TblSalesProductInfo>(invoiceDetail);
        ...
        public async Task<ResponsType> Remove(string invoiceno)
        {
            try
            {
                ...
                return new ResponsType() { Result = "pass", KyValue = invoiceno };
            }
            catch (Exception ex)
            {
                throw ex;
